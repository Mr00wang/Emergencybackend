...
} = require(process.cwd() + '/models/goodsModel');
...
const index = async (req, res) => {
    ...
    if (verifyToken.code === 403) {
        ...
        //3、操作数据库
        let rs = await listModel(getData);
        //4、判断返回
        res.send({
            meta: {
                state: 200,
 ...
 * @param {*} res 
 */
const queryByGoodsName = async (req, res) => {
    //1、接收数据
    let getData = req.query;
    const token = req.headers.token;
    //2、过滤（忽略）
    ...
    if (verifyToken.code === 403) {
        ...
        //3、操作数据库
        let data = await queryModelByGoodsName(getData);
        //4、判断返回
        res.send({
            meta: {
                state: 200,
 ...
 * @param {*} res 
 */
const queryByGoodsType = async (req, res) => {
    //1、接收数据
    let getData = req.query;
    const token = req.headers.token;
    //2、过滤（忽略）
    ...
    if (verifyToken.code === 403) {
        ...
        //3、操作数据库
        let data = await queryModelByGoodsType(getData);
        //4、判断返回
        res.send({
            meta: {
                state: 200,
